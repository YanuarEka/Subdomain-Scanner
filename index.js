function getInputValue() {
  ...
  fetch(url + '=' + anuan, requestOptions)
    ...
    .then((result) => (document.getElementById('demo').innerText = result))
...
allTabsGroup.forEach((tabsGroup) => {
  for (let i = 0; i < tabsGroup.children.length; i++) {
    ...
    tabs.forEach((tab, index) => {
      tab.classList.add('p-4', 'text-center', 'w-full', 'cursor-pointer', 'transition-all', 'duration-200', 'hover:text-blue-500');
      tab.addEventListener('click', () => {
        let old = String(indicator.style.transform || 'translateX(0%)')
          .split('translateX(')
          .filter(Boolean)
          .join('')
          .split('%)')[0];
